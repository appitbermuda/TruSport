...
{
    public class TennisPageViewModel : BaseViewModel
    {
        #region Fields
        ...
        private ObservableCollection<TennisFixture> pastCollection;
        private ObservableCollection<TennisFixture> upcomingCollection;
        ...
        public TennisPageViewModel(INavigation navigation)
        {
            Navigation = navigation;

            PastCollection = new ObservableCollection<TennisFixture>();
            UpcomingCollection = new ObservableCollection<TennisFixture>();
        ...
        public ObservableCollection<TennisFixture> PastCollection
        {
            get { return pastCollection; }
            set { Set(ref pastCollection, value); }
        }

        public ObservableCollection<TennisFixture> UpcomingCollection
        {
        ...
        {
            get { return _isTennis; }
            set { Set(ref _isTennis, value); }
        }
        ...
        internal async void GenerateSource()
        {
            ...
            try
            {
                ...
                var awards = await awardService.GetTennisPlayerOfTheWeek();
                ...
                if (current == NetworkAccess.Internet)
                {
                    ...
                    var fixtures = await fixtureService.GetTennisFixtures();
                    if (fixtures.PastFixtures != null)
                    {
                        ...
                        if (fixtures.PastFixtures.Count() > 0)
                        {
                            ...
                            PastCollection = new ObservableCollection<TennisFixture>(fixtures.PastFixtures.OrderByDescending(e => e.FixtureTime));
                        }
                        else
                        {
                    ...
                    //var upcomingFixtures = await fixtureService.GetUpcomingTennisFixtures();
                    if (fixtures.UpcomingFixtures != null)
                    {
                        ...
                        {
                            UpcomingCollection = new ObservableCollection<TennisFixture>(fixtures.UpcomingFixtures.OrderBy(e => e.FixtureTime));
                            ...
                            foreach (var fixture in UpcomingCollection)
                            {
                                ...
                                if(!String.IsNullOrEmpty(fixture.TennisMatch[0].Player2ID))
                                    event1.Subject = string.Format("{0} & {1} v {2} & {3}",
                                        fixture.TennisMatch[0].Player1.Player.Name,
                                        fixture.TennisMatch[0].Player2.Player.Name,
                                        fixture.TennisMatch[1].Player1.Player.Name,
                                        fixture.TennisMatch[1].Player2.Player.Name);
                                else
                                    event1.Subject = string.Format("{0} v {1}", fixture.TennisMatch[0].Player1.Player.Name, fixture.TennisMatch[1].Player1.Player.Name);
        ...
        internal async Task RefreshPastFixtures()
        {
            try
            {
                var fixtures = await fixtureService.GetPastTennisFixtures();

                if (fixtures != null)
                {
                    var pastFixtures = fixtures.Where(e => e.FixtureTime.AddMinutes(110) <= DateTime.Now);
                    PastCollection = new ObservableCollection<TennisFixture>(pastFixtures);
                }
            }
            catch (Exception ex)
            {
        ...
        internal async Task RefreshUpcomingFixtures()
        {
            try
            {
                var fixtures = await fixtureService.GetUpcomingTennisFixtures();

                if (fixtures != null)
                {
                    var upcomingFixtures = fixtures.Where(e => e.FixtureTime.AddMinutes(110) >= DateTime.Now);

                    UpcomingCollection = new ObservableCollection<TennisFixture>(upcomingFixtures.OrderBy(e => e.FixtureTime));
